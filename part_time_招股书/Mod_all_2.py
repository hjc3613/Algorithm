import os
from modification_2 import modification_2
import json
from tqdm import tqdm
directory = '最新文件抽取结果-fix'
output_dir = '最新文件抽取结果-fix2'
...
for foldername in os.listdir(directory):
    ...
    ok_folder_path = os.path.join(folder_path, "主营业务收入，按产品、地域等划分")
    os.makedirs(ok_folder_path, exist_ok=True)
'''
ignore = ['__pycache__','no_dup','主营业务收入，按产品、地域等划分','员工专业学历','第六节业务与技术']
file_path = []
...
for i,f in tqdm(enumerate(file_path)):
    ...
    output = modification_2(f, f"{output_dir}/{os.path.split(dir)[-1]}/{file_name}")
    if output:

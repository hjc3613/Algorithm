import os
from modification_6 import modification_6
import json
...
for foldername in os.listdir(directory):
    ...
    ok_folder_path = os.path.join(folder_path, "前十大股东")
    os.makedirs(ok_folder_path, exist_ok=True)
'''
ignore = ['__pycache__','no_dup','主营业务收入，按产品、地域等划分',
          '员工专业学历','第六节业务与技术','前五大客户销售','经营成果分析','毛利率',"前十大股东"]
file_path = []
...
for i,f in enumerate(file_path):
    ...
    output = modification_6(f,f"{output_dir}/{os.path.split(dir)[-1]}/{file_name}")
    if output:
        debug[output[0]] = output[1]

with open ('debug_前十大股东.json','w') as f:
